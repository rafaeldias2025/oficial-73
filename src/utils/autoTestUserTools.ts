...
export async function autoTestUserTools(userId: string, profileId: string, challengeId: string) {
  // 1. Teste Sabotadores
  await supabase.from('teste_sabotadores' as any).insert({
    user_id: userId,
  ...
  // 2. Roda da Vida
  await supabase.from('roda_vida' as any).insert({
    user_id: userId,
    areas: {
      saude: 7,
      carreira: 6,
  ...
  // 3. Missão do Dia
  await supabase.from('missao_dia' as any).insert({
    user_id: userId,
  ...
  // 4. Diário de Saúde
  await supabase.from('diario_saude' as any).insert({
    user_id: userId,
    data: new Date().toISOString().slice(0, 10),
    humor: '😊',
    energia: 8,
  ...
  // 5. Desafios
  await supabase.from('user_challenges' as any).insert({
    user_id: profileId,
  ...
  // 6. Dados Físicos
  await supabase.from('dados_fisicos_usuario' as any).insert({
    user_id: userId,
    altura_cm: 175,
    peso_kg: 75,
    idade: 30,
  ...
  // 8. Google Fit
  await supabase.from('google_fit_data' as any).insert({
    user_id: userId,
    steps: 8000,
    calories: 2200,
    distance_km: 6.5,
  ...
  const { data: newCourseRaw } = await supabase.from('courses' as any).insert({
    title: 'Curso Teste Auto',
    description: 'Curso criado pelo autoteste',
    created_by: profileId,
    created_at: new Date().toISOString()
  }).select().single();
  const newCourse: any = newCourseRaw as any;
  if (newCourse && newCourse.id) {
    const { data: newModuleRaw } = await supabase.from('course_modules' as any).insert({
      course_id: newCourse.id,
      title: 'Módulo Teste',
      created_at: new Date().toISOString()
    }).select().single();
    const newModule: any = newModuleRaw as any;
    if (newModule && newModule.id) {
      await supabase.from('course_lessons' as any).insert({
  ...
  // 11. Sessão
  const { data: newSessionRaw } = await supabase.from('sessions' as any).insert({
    user_id: profileId,
    data: new Date().toISOString().slice(0, 10),
    status: 'concluida',
    created_at: new Date().toISOString()
  }).select().single();
  const newSession: any = newSessionRaw as any;
  if (newSession && newSession.id) {
    await supabase.from('session_tools' as any).insert({
      session_id: newSession.id,
      tool: 'Roda da Vida',
      created_at: new Date().toISOString()
    });
    await supabase.from('session_notes' as any).insert({
      session_id: newSession.id,
  ...
  await supabase.from('weekly_evaluations' as any).insert({
    user_id: profileId,
    semana: 29,
  ...
  // 13. Pontuação diária
  await supabase.from('pontuacao_diaria' as any).insert({
    user_id: profileId,
    pontos: 10,
    data: new Date().toISOString().slice(0, 10),
    created_at: new Date().toISOString()
  ...
  await supabase.from('mission_habitos_dia' as any).insert({
    user_id: userId,
    sono_horas: 7,
    agua_litros: 2,
  ...
  await supabase.from('mission_mente_emocoes' as any).insert({
    user_id: userId,
  ...
  await supabase.from('mission_ritual_manha' as any).insert({
    user_id: userId,
  ...
  await supabase.from('beneficios' as any).insert({
    user_id: userId,
